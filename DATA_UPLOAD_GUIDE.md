...
**Note**: The `id` field (e.g., "Alloy-IM-00001") is automatically generated by the system based on material type. You only need to provide the `source` field (e.g., "mp-xxxxx" for Materials Project ID).
...
| `source` | String | ✓ | Data source identifier (e.g., "mp-xxxxx" for Materials Project, or custom ID) |
| `type` | String | ✓ | Classification: "element", "solid-solution", "intermetallic", "amorphous", "interface" |
| `composition` | String | ✓ | Chemical formula with atomic ratios (e.g., "Al2Cu4Ni1") |
| `elements` | Array | ✓ | List of element symbols: `["Al", "Ni", "Cu"]` |
| `atomCount` | Number/Object | ✓ | Total atoms (number) or per-element count (object: `{"Al": 2, "Cu": 4}`) |
| `poscar` | String | ✗ | Path to POSCAR file (e.g., "data/intermetallic/mp-xxxxx/POSCAR") |
| `data` | Array | ✓ | Array of property records (see below) |

**Auto-generated Fields** (do not include in your JSON):
- `id`: Automatically assigned in format "Alloy-XX-#####" (e.g., "Alloy-IM-00001" for intermetallic)
...
```
data/
├── materials.json                    # Main materials database (merged data)
├── intermetallic/                    # Intermetallic compounds data
│   ├── mp-xxxxx/                     # Materials Project ID
...
│   ├── mp-yyyyy/
│   └── ...
├── solid-solution/                   # Solid solution alloys (if separate)
│   ├── source-id-1/
│   │   ├── POSCAR
│   │   └── ...
│   └── ...
└── poscar/                           # Legacy POSCAR files (being phased out)

backend/data/
├── materials.json                    # Copy for backend API
└── materials_intermetallic.json      # Intermetallic data (for reference)
```

### File Organization

- **data/materials.json**: Main database with all materials (auto-generated IDs)
- **data/intermetallic/[source-id]/**: Directory per material source ID
  - `POSCAR`: Crystal structure (default: DFT relaxed structure)
  - `rdf.dat`: Radial distribution function data (optional)
  - `stress_strain.dat`: Mechanical stress-strain curves (optional)
- **backend/data/**: Copies for API serving

### File Naming Convention

- Directory name = `source` field value (e.g., "mp-bbgt", "custom-001")
- POSCAR files are assumed to be from **DFT relaxation** unless specified otherwise
- Additional data files use descriptive names: `rdf.dat`, `stress_strain.dat`
